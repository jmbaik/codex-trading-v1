...
class Kiwoom(QAxWidget):
        ...
        if sRealType == "장시작시간":
            ...
            if sCode in self.jango_dict.keys():
                ...
                if jd['주문가능수량'] > 0 and (meme_rate > 5 or meme_rate < -2):
                    screen_no = self.screen_meme_real_stock if self.condition_stock_dict[sCode]["주문용스크린번호"] is None else self.condition_stock_dict[sCode]["주문용스크린번호"]
                    order_success = self.dynamicCall(
                        "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                        ["신규매도", screen_no, self.account_num, 2, sCode, jd['주문가능수량'], 0, self.realType.SENDTYPE['거래구분']['시장가'], ""]
                    )
                    if order_success == 0:
            ...
            if sCode not in self.jango_dict.keys() and e > 0:
                ...
                order_success = self.dynamicCall(
                    "SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                    ["신규매수", self.condition_stock_dict[sCode]["주문용스크린번호"], self.account_num, 1, sCode, qty, e, self.realType.SENDTYPE['거래구분']['지정가'], ""]
                )
                if order_success == 0:
    ...
    def condition_tr_slot(self, sScrNo, strCodeList, strConditionName, index, nNext):
        ...
        for code in code_list:
            ...
            self.condition_stock_dict[code].update({'주문용스크린번호': self.screen_meme_tr_stock})
    ...
    def condition_real_slot(self, strCode, strType, strConditionName, strConditionIndex):
        ...
        if strType == "I":
            ...
            self.condition_stock_dict[strCode].update({'주문용스크린번호': self.screen_meme_real_stock})
